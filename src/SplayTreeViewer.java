...
public class SplayTreeViewer extends JPanel {
    ...
    private void drawTree(SplayNode root, int depth, int x, int y, Graphics g) {
        ...
        if (root.right != null) {
            dualNode((int) root.value, (int) root.right.value, x, y, (int) (x + 20 * Math.pow(2, depth)), y + 60, g);
            drawTree(root.right, depth - 1, (int) (x + 20 * Math.pow(2, depth)), y + 60, g);
        }
        if (root.left != null) {
            dualNode((int) root.value, (int) root.left.value, x, y, (int) (x - 20 * Math.pow(2, depth)), y + 60, g);
            drawTree(root.left, depth - 1, (int) (x - 20 * Math.pow(2, depth)), y + 60, g);
        }
    }
    ...
    public void paintComponent(Graphics g) {
        if (tree.getRoot() != null) {
            ...
            g.setFont(new Font("Courier New", Font.ITALIC, 20));
            g.drawString(String.valueOf(tree.getRoot().value), CANVAS_WIDTH / 2 + 10, 10 + 30);
            if (tree.size() > 1)
